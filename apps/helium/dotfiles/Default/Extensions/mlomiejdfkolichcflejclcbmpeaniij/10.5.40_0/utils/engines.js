...
async function update(name, { force = false, cache = true } = {}) {
	...
	try {
		...
		if (!data.engines[853]) throw new Error(`Engine "${name}" for "853" engine version not found`);
		...
		if (requiresFullReload) {
			const arrayBuffer = await fetch(data.engines[853].url).then(check).then((res) => res.arrayBuffer());
			engine = deserializeEngine(new Uint8Array(arrayBuffer));
			saveToMemory(name, engine);
			saveToStorage(name, data.engines[853].checksum);
			console.info(`[engines] Engine "${name}" reloaded:`, data.engines[853].checksum);
			return true;
		}
		const diffs = [];
		/**
		...
		*/
		...
		if (updated) {
			console.info(`[engines] Engine "${name}" updated:`, data.engines[853].checksum);
			saveToStorage(name, data.engines[853].checksum);
			return true;
		}
		return false;
