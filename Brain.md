...
### info.sh — The App Manifest
Each app directory has an `info.sh` that is the single source of truth for that app.
Sourced by arch.sh, backup.sh, restore.sh, and detect. Never contains logic.
...
Sources info.sh. Contains optional before_install() and after_install() hooks.
Calls install_with_prompt() from common.sh.

### Essentials Lists
Simple packages with no config to manage. Live in _distro/arch/install.sh
and _de/niri/install.sh as plain arrays. No prompts — run and done.

### common.sh Functions
- detect_distro() — returns arch, deb, unknown
- install_package(name) — distro-aware installer, prefers paru
- install_with_prompt(name, desc, pkg) — shows info, prompts, installs
